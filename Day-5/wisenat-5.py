...
def gridToCsv(grid):
    df = pd.DataFrame(numpy.array(grid))
...
def addLines(grid, lines, diagonal=False):
    # 1 Iteration = 1 Line
    for head, tail in lines:
        ...
        # Y values are equal... iterate through X in matrix selection of Y
            ...
            if diagonal:
                x = arange(head[0], tail[0])
                y = arange(head[1], tail[1])
            else:
                continue
...
def part2():
    '''
    Same as part 1 but we pass a diagonal=True argument to addLines
    '''
    ...
    coords = lines.reshape(-1, lines.shape[-1])
    grid = generateBoundingGrid(coords)
    grid = addLines(grid, lines, diagonal=True)
    # gridToCsv(grid)
    # printGrid(grid)
